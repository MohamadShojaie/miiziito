...
header("Access-Control-Allow-Headers: Content-Type, X-Cashier-Token, X-Super-Admin-Token, Authorization, X-Miiziito-Sandbox, X-Lumier-Sandbox, X-Miiziito-Tenant, X-Lumier-Tenant");
...
function read_json_file($file, $default) {
    ...
    if (function_exists("lumiere_db_operational") && lumiere_db_operational()
...
function save_item_image($uploadsDir, $itemId, $dataUrl) {
    ...
    @chmod($path, 0666);
    $norm = str_replace("\\", "/", $uploadsDir);
    if (preg_match('#/uploads/tenants/([^/]+)$#', $norm, $m)) {
        return "uploads/tenants/" . $m[1] . "/" . $name;
    }
    $web = "uploads/items/";
    $folder = basename($norm);
...
function empty_table_layout() {
    return array("regions" => array(), "states" => array());
}

function sanitize_table_layout($data) {
    if (!is_array($data)) return empty_table_layout();

    $oldFlat = !isset($data["regions"]) && !isset($data["states"]);
    if ($oldFlat) {
        $states = array();
        $hasLegacyStates = false;
    ...
    if (isset($data["regions"]) && is_array($data["regions"])) {
        foreach ($data["regions"] as $region) {
            ...
            $regions[] = array("id" => $id, "name" => $name, "tables" => $tables);
        }
    }
...
function tenant_default_site_settings($nameFa = "کافه", $nameEn = "Cafe") {
    return array(
...
function settings_base_for_file($settingsFile) {
    if (strpos(str_replace("\\", "/", (string) $settingsFile), "/tenants/") === false) {
    ...
    $nameEn = "Cafe";
    if (is_array($raw)) {
        $nameFa = trim((string) (isset($raw["restaurantNameFa"]) ? $raw["restaurantNameFa"] : (isset($raw["restaurantNameEn"]) ? $raw["restaurantNameEn"] : "کافه")));
        if ($nameFa === "") $nameFa = "کافه";
        $nameEn = trim((string) (isset($raw["restaurantNameEn"]) ? $raw["restaurantNameEn"] : $nameFa));
        if ($nameEn === "") $nameEn = $nameFa;
    }
    return tenant_default_site_settings($nameFa, $nameEn);
}
...
function branding_uploads_dir() {
    ...
    if (!empty($uploadsDir) && is_string($uploadsDir) && strpos($uploadsDir, "/tenants/") !== false) {
        $dir = rtrim($uploadsDir, "/") . "/branding";
    } else {
...
function save_branding_image($kind, $dataUrl) {
    ...
    global $uploadsDir;
    if (!empty($uploadsDir) && is_string($uploadsDir) && preg_match('#/tenants/([^/]+)/?$#', rtrim(str_replace("\\", "/", $uploadsDir), "/"), $m)) {
        return "uploads/tenants/" . $m[1] . "/branding/" . $name;
    }
...
function read_site_settings($settingsFile) {
    $raw = read_json_file($settingsFile, array());
    $base = settings_base_for_file($settingsFile);
    return merge_site_settings($base, is_array($raw) ? $raw : array());
}
...
function merge_site_settings($base, $incoming) {
    ...
    foreach ($aliases as $old => $new) {
        if (!isset($incoming[$new]) && isset($incoming[$old])) {
            $incoming[$new] = $incoming[$old];
        }
        if (isset($incoming[$old])) unset($incoming[$old]);
    }
    $out = $base;
...
if ($tenantSlug !== "") {
    ...
    if (!$tenantCafe || !lumiere_tenant_is_live($tenantCafe)) {
        if ($route !== "health") {
            send_json(404, array("error" => "tenant_not_found", "slug" => $tenantSlug));
        }
    } else {
        lumiere_tenant_provision($tenantCafe);
        lumiere_tenant_apply_store(
            $dataDir,
            $ordersFile,
            $menuFile,
            $tablesFile,
