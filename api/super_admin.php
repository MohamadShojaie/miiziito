...
function lumiere_sa_filter_page($items, $searchFields) {
    ...
    foreach ($items as $item) {
        ...
        if ($status !== "" && strtolower((string) (isset($item["status"]) ? $item["status"] : "")) !== $status) {
            continue;
        }
        if ($q !== "") {
            $match = false;
...
function lumiere_super_admin_handle($method, $route, $id, $body, $headers) {
    ...
    if ($route === "sa-cafes" && $method === "GET") {
        ...
        return array(
            "status" => 200,
            "body" => lumiere_sa_filter_page($cafes, array("name", "ownerName", "email", "phone", "id", "subscriptionId")),
        );
    }
    ...
    if ($route === "sa-support" && $method === "GET") {
        ...
        return array(
            "status" => 200,
            "body" => lumiere_sa_filter_page($tickets, array("id", "subject", "tenantId", "status", "priority")),
        );
    }
    ...
    if ($route === "sa-support-item" && $itemId !== "") {
        ...
        if ($idx < 0) return array("status" => 404, "body" => array("error" => "not_found"));
        if ($method === "GET") {
            return array("status" => 200, "body" => array("ticket" => $tickets[$idx]));
        }
        if ($method === "POST") {
            ...
            if ($action === "reply") {
                ...
                $ticket["status"] = (string) (isset($body["status"]) ? $body["status"] : "in_progress");
            } else {
